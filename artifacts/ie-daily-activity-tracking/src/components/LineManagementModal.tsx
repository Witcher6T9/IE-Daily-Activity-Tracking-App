...
export const LineManagementModal: React.FC<LineManagementModalProps> = ({
  ...
  const [newFloor, setNewFloor] = useState('Floor 01 / Unit A');
  ...
  const handleAddLine = (e: React.FormEvent) => {
    ...
    const created: ProductionLine = {
      ...
      floor: newFloor.trim() || 'Floor 01 / Unit A',
    ...
    onSaveLines(updated);
    setNewLineNo('');
    setSaveNote(`Line ${created.lineNo} added successfully ✓`);
  ...
  const filteredLines = localLines.filter(line => {
    ...
    return line.lineNo.toLowerCase().includes(q) || line.floor.toLowerCase().includes(q);
  });
  ...
  return (
    <div className="max-w-4xl mx-auto p-4 sm:p-6 md:p-8">
      ...
      <div className="bg-white rounded-3xl border border-slate-200/90 p-5 sm:p-6 shadow-2xs mb-8">
        ...
        <form onSubmit={handleAddLine} className="grid grid-cols-1 sm:grid-cols-4 gap-4 items-end">
          <div>
          ...
          <div>
            <label className="block text-xs font-bold text-slate-700 mb-1">Floor / Unit / Building</label>
            <input
              type="text"
              placeholder="e.g. Floor 02 / Unit B"
              value={newFloor}
                    ...
                    {isEditing ? (
                      <div className="space-y-1">
                        <input
                          ...
                          className="border border-blue-300 rounded-lg px-2 py-1 text-xs w-56 font-semibold"
